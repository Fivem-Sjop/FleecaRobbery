            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Pak het Cash", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Pak het Cash", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Pak het Cash", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" or PlayerData.job.name == "kmar" then
        exports["mythic_notify"]:DoHudText("inform", "Een bank alarm is afgegaan!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    exports["mythic_notify"]:DoHudText("Pas Op!", "De kluisdeur sluit in 30 seconden!")
    Citizen.Wait(30000)
    exports["mythic_notify"]:DoHudText("Pas Op!", "Kluisdeur sluit!")
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports["mythic_notify"]:DoHudText("Succesvol", "Hack is compleet!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    exports["mythic_notify"]:DoHudText("Pas op!", "Je hebt 2 minuten tot het beveiligingssysteem af gaat.")
...
function SecondDoor(data, key)
    ...
    exports["mythic_notify"]:DoHudText("Succesvol", "Toegang Toegekend!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "[~r~E~w~] Gebruik Beveiligd ID Kaart", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports["mythic_notify"]:DoHudText("FOUTMELDING", "Je hebt geen Beveiligd ID Kaart.")
                        end
                    end)
                end
            end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~r~E~w~] Start bankoverval", 0.40)
