        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Deze bank is reeds beroofd. Je moet even wachten "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Deze bank wordt momenteel beroofd.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, " Je hebt geen Gehackte Toegangs Kaart.")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "Er is niet genoeg politie in de stad.")
    end
end)
